...
class CobranzaExport implements FromCollection, WithHeadings, WithStyles, ShouldAutoSize, WithMapping, WithEvents
{
    ...
    public function collection()
    {
        $recibos = DB::table('z_control_recibos_ot as ot')
            ...
            ->select(
                ...
                'i.nota AS nota',
                'i.monto_banco',
                DB::raw('CASE WHEN i.monto_banco IS NOT NULL AND i.monto_banco <> "" THEN i.monto_banco - ot.importe_bs ELSE NULL END AS diferencia'),// Calcula la diferencia solo si monto_banco no es vacío
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        foreach ($this->collection() as $recibos) {
            ...
            $cellDiferencia= $recibos->diferencia;
            $banderaCell = "N$rowIndex";
            $colorDiferencia="H$rowIndex";
            if ($cellValue == 2) {
            ...
            if($cellDiferencia>0){
                $sheet->getStyle($colorDiferencia)->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB(Color::COLOR_GREEN);
            }
            if($cellDiferencia<0)
            {
                $sheet->getStyle($colorDiferencia)->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB(Color::COLOR_RED);
            }
            $rowIndex++;
        }
    }
