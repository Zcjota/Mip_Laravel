                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            <div class="input-group search-bar">
                                <select v-model="tipoFecha" class="form-control square" style="width: 100px;" @change="validateDate">
                ...
                <div class="card-body">
                    <div style="overflow-x:auto;">
                        <div class="panel-body table-responsive">
                            <div id="bodyListado">
                                <table id="mainTableSG" class="table table-bordered">
                                    <thead>
                                        <tr>
                                            <th class="sticky-column-1">Nro</th>
                                            <th class="sticky-column-2">Cliente</th>
                                            <th class="sticky-column-3">Precio</th>
                                            <th class="sticky-column-4">Fecha Servicio</th>
                                            <th>Tipo Pago</th>
                                            <th>Plazo</th>
                                    ...
                                    <tbody>
                                        <tr v-for="item in items" :key="item.codigo">
                                            <td class="sticky-column-1">{{ item.nro }}</td>
                                            <td class="sticky-column-2">{{ item.cliente }}</td>
                                            <td class="sticky-column-3">{{ item.precio }}</td>
                                            <td class="sticky-column-4">{{ item.fechaser }}</td>
                                            <td>{{ item.tp }}</td>
                                            <td>{{ item.plazo }}</td>
...
export default {
    ...
    methods: {
        ...
        fetchData(page = 1) {
            axios.get('/api/registroventas/listar', {
                params: {
...
.sticky-column-4 {
    position: sticky;
    left: 0;
    background-color: rgb(23, 71, 228); /* Asegúrate de que el fondo sea blanco */
    z-index: 1;
    border-right: 1px solid #080808;
    font-weight: bold; /* Ajusta el estilo de la letra aquí */
    color: #000; /* Ajusta el color de la letra aquí */
}

/* Ajusta las posiciones left para cada columna */
.sticky-column-2 {
    left: 60px; /* Ajusta según el ancho de la primera columna */
}

.sticky-column-3 {
    left: 120px; /* Ajusta según el ancho de las dos primeras columnas */
}

.sticky-column-4 {
    left: 180px; /* Ajusta según el ancho de las tres primeras columnas */
}
</style>
